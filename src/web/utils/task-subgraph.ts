 ...
 * Unlike the old dependency-only cut, this walks every typed edge of the global graph
 * payload — ParentOf (父任务/子任务), DependsOn (依赖于), BelongsToMilestone (里程碑) —
 * in both directions, so the modal shows the same picture the /graph page would, just
 * centered on the open task.
 */
...
export function buildRelationshipSubgraph(
	...
	return {
		...
		edges: payload.edges.filter((edge) => depthById.has(edge.from) && depthById.has(edge.to)),
	};
}
