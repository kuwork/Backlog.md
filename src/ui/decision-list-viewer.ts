...
/**
 * Open an interactive two-pane decision browser: list on the left, details on
 * the right. Left/right arrows switch focus; up/down / j k navigate the list
 * or scroll the detail pane depending on which pane is active. Esc / q / Ctrl+C
 * exit.
 */
export async function runDecisionListViewer(decisions: Decision[], core: Core): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		const listBox = list({
			...
			bottom: footerHeight,
			border: "line",
			items: decisions.map((d) => `${d.id} - ${d.title}`),
			keys: true,
			mouse: false,
			scrollable: true,
			style: {
		...
		screen.key(["?"], () => {
			if (helpOpen) return false;
			helpOpen = true;
			void openHelpPopup(screen, "decision-list").finally(() => {
				helpOpen = false;
				screen.render();
			});
			return false;
