...
export async function viewTaskEnhanced(
	...
	function refreshDetailPane() {
		...
		const detailContent = generateDetailContent(currentSelectedTask, resolveMilestoneLabel, buildReadinessGraph());
	...
	const openCurrentTaskInEditor = async () => {
		...
		try {
			...
			if (result.reason === "editor_failed") {
				showTransientHelp(` {red-fg}Editor exited with an error; ${noun.plain} was not modified.{/}`);
				return;
			}
			if (result.reason === "not_found") {
				showTransientHelp(` {red-fg}${noun.titled} ${selectedTask.id} was not found on this branch.{/}`);
				return;
			}
			...
			applyFilters();
			if (result.changed) {
				showTransientHelp(` {green-fg}${noun.titled} ${result.task?.id ?? selectedTask.id} marked modified.{/}`);
				return;
			}
