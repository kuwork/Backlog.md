...
/**
 * Validate that explicit config values are well-formed enough to publish. The config parser
 * already rejects values it cannot hold; this guard catches wrong-typed explicit values that
 * the parser would silently ignore (e.g. a scalar where a list belongs) and keeps the last
 * good config cached instead of replacing it with a broken one.
 *
 * Adapted to the current fork's supported config keys.
 */
function hasValidExplicitValues(content: string, config: BacklogConfig): boolean {
	for (const rawLine of content.split(/\r?\n/)) {
		...
		// default_assignee is intentionally not in ARRAY_CONFIG_KEYS because it also accepts a
		// single scalar or a block sequence; if the value is present but the parser could not hold
		// it, treat the config as malformed so the last good config stays cached.
		if (key === "default_assignee" && value && config.defaultAssignee === undefined) return false;

		if (key === "definition_of_done") {
			if (value.startsWith("[") && !value.endsWith("]")) return false;
			if (config.definitionOfDone === undefined) return false;
		}
