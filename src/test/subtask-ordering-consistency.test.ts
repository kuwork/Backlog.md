...
// compares ids as plain strings puts TASK-1.10 in front of TASK-1.2 (issue 953).
...
describe("subtask ordering consistency", () => {
	...
	it("agrees with the CLI plain and JSON task lists", async () => {
		const plain = await $`bun ${CLI_PATH} task list --plain`.cwd(TEST_DIR).quiet();
		expect(plain.exitCode).toBe(0);
		expect(idsFromPlainOutput(plain.stdout.toString())).toEqual(EXPECTED_ORDER);

		const json = await $`bun ${CLI_PATH} task list --json`.cwd(TEST_DIR).quiet();
