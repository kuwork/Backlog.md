...
describe("task edit with several task IDs", () => {
	...
	it("moves every listed task to the target status", async () => {
		const result = await $`bun ${cliPath} task edit task-1 task-2 task-3 -s "In Progress"`
			.cwd(TEST_DIR)
			.nothrow()
			.quiet();
	...
	it("moves the tasks that succeed and names the ones that fail", async () => {
		const result = await $`bun ${cliPath} task edit task-1 task-999 task-3 -s "In Progress"`
			.cwd(TEST_DIR)
			.nothrow()
			.quiet();
	...
	it("rejects a per-task flag when the user passes more than one ID", async () => {
		const result = await $`bun ${cliPath} task edit task-1 task-2 -t "New title"`.cwd(TEST_DIR).nothrow().quiet();
	...
	it("keeps the shared flags available for a batch", async () => {
		const result = await $`bun ${cliPath} task edit task-1 task-2 --priority high --add-label triage`
			.cwd(TEST_DIR)
			.nothrow()
			.quiet();
	...
	it("treats clearing flags such as --unassign as changes a batch can apply", async () => {
		const core = new Core(TEST_DIR);
		for (const id of ["task-1", "task-2"]) {
			const task = await core.filesystem.loadTask(id);
			if (task) await core.filesystem.saveTask({ ...task, assignee: ["@sara"] });
		}

		const result = await $`bun ${cliPath} task edit task-1 task-2 --unassign`.cwd(TEST_DIR).nothrow().quiet();

		expect(result.exitCode).toBe(0);
		for (const id of ["task-1", "task-2"]) {
			expect((await core.filesystem.loadTask(id))?.assignee).toEqual([]);
		}
	});

	it("prints one outcome line per task", async () => {
		const result = await $`bun ${cliPath} task edit task-1 task-2 -s Done`.cwd(TEST_DIR).nothrow().quiet();
	...
	it("keeps the single-ID output unchanged", async () => {
		const result = await $`bun ${cliPath} task edit task-1 -s Done`.cwd(TEST_DIR).nothrow().quiet();

		expect(result.exitCode).toBe(0);
		expect(result.stdout.toString()).toContain("Updated task TASK-1");
	});

	it("prints the full task body for a single ID with --plain", async () => {
		const result = await $`bun ${cliPath} task edit task-1 -s Done --plain`.cwd(TEST_DIR).nothrow().quiet();

		expect(result.exitCode).toBe(0);
		expect(result.stdout.toString()).toContain("Task TASK-1 - First");
	});

	it("refuses several IDs with no field flag instead of editing only the first", async () => {
		const result = await $`bun ${cliPath} task edit task-1 task-2 task-3`.cwd(TEST_DIR).nothrow().quiet();
	...
	it("treats --plain alone as an output choice, not a change to apply to the batch", async () => {
		const result = await $`bun ${cliPath} task edit task-1 task-2 --plain`.cwd(TEST_DIR).nothrow().quiet();
	...
	it("counts a repeated ID once", async () => {
		const result = await $`bun ${cliPath} task edit task-1 TASK-1 -s Done`.cwd(TEST_DIR).nothrow().quiet();
	...
	it("rejects task-specific section flags for a batch", async () => {
		for (const flags of [
			...
			const result = await $`bun ${cliPath} task edit task-1 task-2 ${flags}`.cwd(TEST_DIR).nothrow().quiet();
	...
	it("keeps a bare number and an explicitly prefixed ID distinct", async () => {
		...
		const result = await $`bun ${cliPath} task edit 1 JIRA-1 -s Done`.cwd(TEST_DIR).nothrow().quiet();
	...
	it("edits a task once when a bare number and the prefixed ID alias it in one batch", async () => {
		// Under a custom prefix the bare number only becomes the prefixed ID at resolution time, so
		// the batch must collapse on the resolved identity instead of saving the same task twice.
		const prefixedDir = createUniqueTestDir("test-batch-edit-prefix");
		try {
			await rm(prefixedDir, { recursive: true, force: true });
			await mkdir(prefixedDir, { recursive: true });
			await $`bun ${cliPath} init "Prefixed Batch Project" --no-git --defaults --integration-mode none --task-prefix JIRA`
				.cwd(prefixedDir)
				.quiet();
			const core = new Core(prefixedDir);
			await createTask(core, "JIRA-1", "Prefixed target");

			const result = await $`bun ${cliPath} task edit 1 JIRA-1 -s Done`.cwd(prefixedDir).nothrow().quiet();
			...
			expect((await core.filesystem.loadTask("JIRA-1"))?.status).toBe("Done");
		} finally {
			try {
				await safeCleanup(prefixedDir);
			} catch {}
		}
	});
});
