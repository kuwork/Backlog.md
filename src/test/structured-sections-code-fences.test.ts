...
describe("structured sections blank-line normalization with code fences", () => {
	...
	// Kept from the fork's pre-upgrade fence suite: the cases the CommonMark scanner
	// does not cover on its own. The same-family marker example that suite used is gone
	// on purpose - such a payload is now rejected at the input boundary, see
	// src/test/section-marker-safety.test.ts.
	describe("fork coverage kept from the pre-upgrade suite", () => {
