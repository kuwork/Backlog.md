...
describe("BacklogServer batch move endpoint", () => {
	beforeEach(async () => {
		...
		// A column that already has cards, so a positional move has somewhere to land.
		await core.createTask(makeTask({ id: "task-3", status: "Done", ordinal: 1000 }), false);
		await core.createTask(makeTask({ id: "task-4", status: "Done", ordinal: 2000 }), false);
	...
	it("places a batch at the named order instead of appending it", async () => {
		const { status, payload } = await postMove({
			taskIds: ["task-1"],
			targetStatus: "Done",
		...
		expect(status).toBe(200);
		expect(payload.success).toBe(true);
		const placed = await core.filesystem.loadTask("task-1");
		expect(placed?.status).toBe("Done");
		// Seeded between its new neighbours, exactly as a single-task reorder seeds its midpoint.
		expect(placed?.ordinal).toBeGreaterThan(1000);
		expect(placed?.ordinal).toBeLessThan(2000);
	});

	it("appends a batch that names no order", async () => {
		const { status } = await postMove({ taskIds: ["task-1"], targetStatus: "Done" });

		expect(status).toBe(200);
		const appended = await core.filesystem.loadTask("task-1");
		expect(appended?.status).toBe("Done");
		expect(appended?.ordinal).toBeGreaterThan(2000);
	});

	it("keeps a task the named order leaves out in its own column and reports it", async () => {
		const { status, payload } = await postMove({
	...
	it("rejects an order that leaves out a task it is moving", async () => {
		const { status, payload } = await postMove({
			taskIds: ["task-1", "task-2"],
			targetStatus: "Done",
		...
		expect(payload.error).toContain("orderedTaskIds must include every task being moved");
	});

	it("rejects an order that names a task twice", async () => {
		const { status, payload } = await postMove({
			taskIds: ["task-1"],
			targetStatus: "Done",
		...
		expect(status).toBe(400);
		expect(payload.error).toContain("Duplicate task ID");
	});
