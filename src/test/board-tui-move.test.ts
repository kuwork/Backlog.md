...
/**
 * Press a key through the same path blessed uses on a real keypress: the keypress event
 * plus the `key <full>` event the screen's key registrations listen on. `full` is the
 * discriminator, so `S-m` / `S-down` only reach handlers registered under that exact name.
 */
...
/**
 * Task ids of the rows the board currently highlights, read from the list widgets' own
 * selection: this is the move-mode bar the recruitment highlight reuses, as opposed to the
 * `►` marker the moving set carries.
 */
function highlightedRowIds(root: TreeWidget): string[] {
	const ids: string[] = [];
	const visit = (node: TreeWidget) => {
		if (node.type === "list" && typeof node.selected === "number") {
			const id = node.items?.[node.selected]?.content?.match(/TASK-\d+/)?.[0];
			if (id) ids.push(id);
		}
		for (const child of node.children ?? []) visit(child);
	};
...
/**
 * Poll until a write the board confirmed has landed. A persisted board move costs seconds here
 * (the content store refresh it goes through is far slower than the default retry window allows),
 * and the key handlers cannot be awaited from a test, so the assertion has to poll instead.
 */
const waitForPersisted = <T>(fn: () => Promise<T>): Promise<T> => retry(fn, 40, 250);

let TEST_DIR: string;
let core: Core;

beforeEach(async () => {
	// Outside the repository on purpose: inside it every store refresh walks up to this project
	// and runs `git fetch`, which costs seconds here and makes the persistence assertions race it.
	TEST_DIR = await mkdtemp(join(tmpdir(), "board-tui-move-"));
	core = new Core(TEST_DIR);
	await initializeTestProject(core, "Board TUI Move");
...
async function withBoard(
	run: (context: {
		...
		rows: () => string[];
		footer: () => string;
		/** Task ids of the rows carrying the board's own row highlight. */
		highlighted: () => string[];
		quit: () => Promise<void>;
	...
	try {
		...
		await run({
			...
			highlighted: () => highlightedRowIds(screen as unknown as TreeWidget),
			quit,
...
describe("TUI board multi-select mover", () => {
	...
	it("walks the highlight with Shift+Down without moving the grabbed task", async () => {
		await withBoard(({ screen, rows, highlighted }) => {
			pressKey(screen, "m");
			// The grabbed task's ghost is the top row and the highlight starts on it.
			expect(highlighted()).toEqual(["TASK-1"]);

			// The highlight is the existing move-mode row bar relocated off the ghost, so it
			// walks the target column's rows while the ghost (and only the ghost) stays marked.
			pressKey(screen, "S-down");
			expect(highlighted()).toEqual(["TASK-2"]);
			pressKey(screen, "S-down");
			expect(highlighted()).toEqual(["TASK-3"]);
	...
	it("treats a confirm that lands the set where it already sits as a no-op", async () => {
		await withBoard(async ({ screen, footer, quit }) => {
			let moveCalls = 0;
			...
			// Recruit the task directly below the grabbed one and confirm in place: the block
			// preview lands the set exactly where it already is, so nothing may be written.
			pressKey(screen, "m");
			pressKey(screen, "S-m");
			pressKey(screen, "enter");
			await Bun.sleep(50);
			...
			expect(footer()).not.toContain("MOVE MODE");
			expect((await core.filesystem.loadTask("TASK-2"))?.ordinal).toBe(2000);
			await quit();
		});
	});
