...
/** Blessed screens extend EventEmitter; narrow to the events this test drives. */
type EmittingScreen = ScreenInterface & {
	emit(event: string): void;
	destroy(): void;
};

/**
 * Drives the board's "new task" handler and returns the task it persisted.
...
async function createTaskFromBoardKeypress(options?: { core?: Core }): Promise<Task | null> {
	...
	let resolveCreated: (task: Task | null) => void = () => {};
	// Resolved by the composer once its persist call completes — no wall-clock polling.
	const createdPromise = new Promise<Task | null>((resolve) => {
	...
	try {
		const boardPromise = renderBoardTui([], ["To Do", "Done"], "horizontal", 20, {
			...
			taskComposer: async ({ persist }) => {
				...
				const created = await persist(input);
		...
		// The create handler releases its modal guard one microtask after persist settles; one
		// macrotask turn guarantees the quit key is no longer guarded when emitted.
