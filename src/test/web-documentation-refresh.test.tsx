...
// Same heading, different body: an edit that must not be mistaken for an unrelated refresh.
const EDITED_SOURCE = "## A1: Section Title\n\nEdited body text";
const DOC_PATH = "/documentation/doc-1/alpha#a1-section-title";
const BARE_PATH = "/documentation/doc-1#a1-section-title";
// Edit mode is opened from the query string, so this entry carries no hash.
const EDIT_PATH = "/documentation/doc-1/alpha?edit=true";

function docFixture(source: string = DOC_SOURCE): BacklogDocument {
	return {
		id: "doc-1",
		title: "Alpha",
		type: "other",
...
describe("DocumentationDetail refresh while a hash link is open", () => {
	...
	// Body the server would return for the open document; tests mutate it to stand in for an edit
	// made outside the app.
	let servedSource = DOC_SOURCE;
	...
	beforeEach(() => {
		...
		servedSource = DOC_SOURCE;
		globalThis.fetch = (async (input: RequestInfo | URL) => {
			if (String(input).includes("/api/docs/")) docFetchCount += 1;
			return new Response(JSON.stringify(docFixture(servedSource)), {
				status: 200,
	...
	it("reloads the body when an external edit changes its fingerprint", async () => {
		await renderDocs([docFixture()]);
		const heading = container?.querySelector("h2#a1-section-title");
		expect(docFetchCount).toBe(1);

		// An edit outside the app: the stored body changed, so the parent's docs array carries a new
		// fingerprint and the viewer has to pick the change up without a page reload.
		servedSource = EDITED_SOURCE;
		await renderDocs([docFixture(EDITED_SOURCE)]);

		expect(docFetchCount).toBe(2);
		expect(container?.textContent).toContain("Edited body text");
		// The reload was silent, so the rendered heading was never unmounted and the reader keeps
		// the anchor they arrived on.
		expect(container?.querySelector("h2#a1-section-title")).toBe(heading);
		expect(currentLocation.hash).toBe("#a1-section-title");
	});

	it("ignores a refresh whose fingerprint matches the rendered body", async () => {
		await renderDocs([docFixture()]);
		const heading = container?.querySelector("h2#a1-section-title");
		expect(docFetchCount).toBe(1);

		// Same body in fresh objects: the parent refreshed on an unrelated websocket broadcast.
		await renderDocs([docFixture()]);
	...
	it("keeps an in-progress edit when the stored body changes underneath", async () => {
		await renderDocs([docFixture()], EDIT_PATH);
		expect((container?.querySelector("textarea") as HTMLTextAreaElement | null)?.value).toBe(DOC_SOURCE);

		servedSource = EDITED_SOURCE;
		await renderDocs([docFixture(EDITED_SOURCE)], EDIT_PATH);
