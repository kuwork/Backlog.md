...
| Set due date | `backlog task edit 7 --due-date 2026-06-15`         |
...
> **Note on actual dates:** `actualStart` and `actualEnd` are automatically populated when you change a task's status (to "In Progress" and a terminal status like "Done", respectively). Set them manually only when you need to override or record a different time.
>
> Each date field has a matching `--clear-*` flag to remove it.
...
| Edit dates | `backlog milestone edit "Release 1.0" --due-date 2026-06-15 --planned-start 2026-06-01 --planned-end 2026-06-30` |
| Clear milestone dates | `backlog milestone edit "Release 1.0" --clear-due-date --clear-planned-start --clear-planned-end` |
