...
- Web, real browser (server on 6611 + Chromium, throwaway decision deleted afterwards): the select appears in edit mode with the current value and aria-label 状态; a status-only save left the file body untouched and the preview badge moved Accepted -> Rejected after the save; a body+status save stored both with a single set of headings.
