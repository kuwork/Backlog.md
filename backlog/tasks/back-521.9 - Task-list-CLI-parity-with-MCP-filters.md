...
<!-- SECTION:NOTES:BEGIN -->
Started implementation on branch `tasks/back-521-agent-cli-workflow`. Initial scope: add `labels`, `search`, and positive `limit` support to `backlog task list`, update public help schema/examples and new CLI instruction guides, and cover behavior with focused CLI/instruction tests.
