...
```bash
# 30 搜索包含 "api" 且状态为 In Progress 的任务
...
```bash
# 39 默认启动：端口 6420，自动打开系统默认浏览器
backlog browser

# 40 指定自定义端口
backlog browser --port 8080

# 41 启动但不自动打开浏览器（适合远程服务器或后台运行）
...
### 41.1.1 导出为 Markdown 表格

```bash
# 42 导出到默认文件 Backlog.md
...
```bash
# 55 交互式查看（TUI 弹窗渲染 Markdown）
backlog doc view doc-5

# 56 纯文本输出（适合 AI 代理或脚本解析）
...
backlog init my-project
# 65 交互式向导中出现选项：
# 66 "Install AI agent instructions?"
```

选择安装后，Backlog.md 会根据你的选择生成对应的指令文件。

#### 66.0.0.1 手动更新已有项目的指令
...
#### 66.0.1.4 .github/copilot-instructions.md
...
# 70 显示：task-1  task-2  task-3  doc-1  ...
```
...
# 72 显示：bug  feature  docs  backend
```
...
```bash
# 74 查找全局安装的 backlog.md 包路径
npm root -g
# 75 补全脚本位于：
# 76 <npm-root>/backlog.md/completions/backlog.bash
# 77 <npm-root>/backlog.md/completions/backlog.zsh
# 78 <npm-root>/backlog.md/completions/backlog.fish
# 79 <npm-root>/backlog.md/completions/backlog.ps1
```
