...
| `dueDate` | 截止日期（ISO 格式） | `"2026-05-20"` |
| `plannedStart` | 计划开始日期（ISO 格式） | `"2026-05-10"` |
| `plannedEnd` | 计划结束日期（ISO 格式） | `"2026-05-18"` |
...
# 23 编辑任务日期
backlog task edit back-10 --due-date 2026-06-15 --planned-start 2026-06-01

# 24 清空日期字段
backlog task edit back-10 --clear-due-date --clear-planned-start
```

日期格式统一为 `YYYY-MM-DD`（date-only）。在交互式 TTY 环境下，创建/编辑向导会提示输入日期。

#### 24.0.0.1 Web UI 中的日期编辑

在任务详情弹窗的侧边栏中，提供三个日期输入框：
...
```bash
# 29 搜索包含 "api" 且状态为 In Progress 的任务
...
```bash
# 38 默认启动：端口 6420，自动打开系统默认浏览器
backlog browser

# 39 指定自定义端口
backlog browser --port 8080

# 40 启动但不自动打开浏览器（适合远程服务器或后台运行）
...
### 40.1.1 导出为 Markdown 表格

```bash
# 41 导出到默认文件 Backlog.md
...
```bash
# 54 交互式查看（TUI 弹窗渲染 Markdown）
backlog doc view doc-5

# 55 纯文本输出（适合 AI 代理或脚本解析）
...
# 60 清空日期字段
backlog milestone edit M1 --clear-due-date --clear-planned-end
```
...
backlog init my-project
# 64 交互式向导中出现选项：
# 65 "Install AI agent instructions?"
```

选择安装后，Backlog.md 会根据你的选择生成对应的指令文件。

#### 65.0.0.1 手动更新已有项目的指令
...
#### 65.0.1.4 .github/copilot-instructions.md
...
# 69 显示：task-1  task-2  task-3  doc-1  ...
```
...
# 71 显示：bug  feature  docs  backend
```
...
```bash
# 73 查找全局安装的 backlog.md 包路径
npm root -g
# 74 补全脚本位于：
# 75 <npm-root>/backlog.md/completions/backlog.bash
# 76 <npm-root>/backlog.md/completions/backlog.zsh
# 77 <npm-root>/backlog.md/completions/backlog.fish
# 78 <npm-root>/backlog.md/completions/backlog.ps1
```
