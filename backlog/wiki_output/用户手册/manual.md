...
```bash
# 28 搜索包含 "api" 且状态为 In Progress 的任务
...
```bash
# 37 默认启动：端口 6420，自动打开系统默认浏览器
backlog browser

# 38 指定自定义端口
backlog browser --port 8080

# 39 启动但不自动打开浏览器（适合远程服务器或后台运行）
...
### 39.1.1 导出为 Markdown 表格

```bash
# 40 导出到默认文件 Backlog.md
...
```bash
# 53 交互式查看（TUI 弹窗渲染 Markdown）
backlog doc view doc-5

# 54 纯文本输出（适合 AI 代理或脚本解析）
...
backlog init my-project
# 63 交互式向导中出现选项：
# 64 "Install AI agent instructions?"
```

选择安装后，Backlog.md 会根据你的选择生成对应的指令文件。

#### 64.0.0.1 手动更新已有项目的指令
...
#### 64.0.1.4 .github/copilot-instructions.md
...
# 68 显示：task-1  task-2  task-3  doc-1  ...
```
...
# 70 显示：bug  feature  docs  backend
```
...
```bash
# 72 查找全局安装的 backlog.md 包路径
npm root -g
# 73 补全脚本位于：
# 74 <npm-root>/backlog.md/completions/backlog.bash
# 75 <npm-root>/backlog.md/completions/backlog.zsh
# 76 <npm-root>/backlog.md/completions/backlog.fish
# 77 <npm-root>/backlog.md/completions/backlog.ps1
```
